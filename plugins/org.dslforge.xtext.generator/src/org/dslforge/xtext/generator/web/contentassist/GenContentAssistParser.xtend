...
class GenContentAssistParser implements IWebProjectGenerator{
	...
	//	xtextGenerator.pathRtProject = pathRtProject
		...
		try {				
				...
				// WORKAROUND!! the Xtext generator adds the default UI modules, remove them as we use our own web modules
				deleteFile(projectName + "/" + "src-gen" + "/" + basePath + "/"+ "Abstract" + grammarShortName.toFirstUpper + "UiModule.java")
				deleteFile(projectName + "/" + "src" + "/" + basePath + "/"+ grammarShortName.toFirstUpper + "UiModule.java")
