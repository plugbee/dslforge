 ...
 ******************************************************************************/
package org.dslforge.workbench.theme.managers;
...
import org.dslforge.workbench.theme.builder.CoolbarLayerBuilder;
...
public class CoolBarManager extends CoolBarManager2 {
		...
		public void widgetSelected(final SelectionEvent e) {
			...
			if (widget != null && widget instanceof ToolItem
				...
				if (widget.getData(RWT.CUSTOM_VARIANT) != null) {
					IContributionItem item = (IContributionItem) widget
							.getData();
	...
	public CoolBarManager() {
		dummyBuilder = new DummyBuilder(null,
				ILayoutSetConstants.SET_ID_COOLBAR);
	}
		...
		if ((isDirty() || force) && getControl2() != null) {
			...
			while (visibleItemItr.hasNext()) {
				...
				if (sourceIndex < toolItems.length) {
					destinationItem = (IContributionItem) toolItems[sourceIndex]
							.getData();
				} else {
	...
	private int getChildrenSize(final ToolBar toolbar) {
		int result = 0;
		FormData spacing = dummyBuilder
				.getPosition(ILayoutSetConstants.COOLBAR_SPACING);
		if (spacing != null) {
	...
	private FormData getOverflowButtonLayoutData() {
		...
		FormData fdOverFlowButton = dummyBuilder
				.getPosition(ILayoutSetConstants.COOLBAR_BUTTON_POS);
		if (image != null) {
	...
	private void createOverflowLayer() {
		ElementBuilder layerBuilder = new CoolbarLayerBuilder(
				overflowParent.getParent(), ILayoutSetConstants.SET_ID_OVERFLOW);
		...
		if (adapter != null) {
			// position the layer
			FormData fdLayer = (FormData) overflowLayer.getParent()
					.getLayoutData();
	...
	private void fillOverflowToolbar() {
		if (overflowToolbar == null) {
			...
			overflowToolbarParent = new ScrolledComposite(overflowLayer,
					SWT.V_SCROLL);
			DummyBuilder builder = new DummyBuilder(null,
					ILayoutSetConstants.SET_ID_OVERFLOW);
			FormData pos = builder
					.getPosition(ILayoutSetConstants.OVERFLOW_POS);
			...
			overflowToolbar.setData(RWT.CUSTOM_VARIANT, HEADER_OVERFLOW_VARIANT);
