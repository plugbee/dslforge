 ...
 ******************************************************************************/
package org.dslforge.workbench.theme.stacks;
...
import org.dslforge.workbench.theme.Messages;
import org.dslforge.workbench.theme.builder.StackPresentationBuider;
...
public class ViewStackPresentation extends ConfigurableStack {
	...
	private void createToolBarBg() {
		...
		Image bg = stackBuilder
				.getImage(ILayoutSetConstants.STACK_VIEW_TOOLBAR_BG);
			...
			if (viewMenu != null) {
				if (viewMenuButton == null) {
					...
					Image icon = stackBuilder
							.getImage(ILayoutSetConstants.STACK_VIEW_MENU_ICON);
	...
	private void makePartButtonActive(final IPresentablePart part) {
		...
		if (object instanceof Composite) {
			...
			Color bg = stackBuilder
					.getColor(ILayoutSetConstants.STACK_BUTTON_INACTIVE);
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (child instanceof Button) {
					...
					FormData pos = stackBuilder
							.getPosition(ILayoutSetConstants.STACK_BUTTON_TOP);
	...
	private void checkTabBg() {
		...
		if (tabBg == null && tabBar != null) {
			...
			FormData fdLayout = stackBuilder
					.getPosition(ILayoutSetConstants.STACK_TABBG_POS);
	...
	private void createConfArea(FormData fdTabBg) {
		...
		if (configAction != null) {
			...
			Image confBg = stackBuilder
					.getImage(ILayoutSetConstants.STACK_CONF_BG_INACTIVE);
			...
			Image cornerImage = stackBuilder
					.getImage(ILayoutSetConstants.STACK_INACTIVE_CORNER);
			...
			Image confImage = stackBuilder
					.getImage(ILayoutSetConstants.STACK_CONF_INACTIVE);
			...
			FormData fdConfButton = stackBuilder
					.getPosition(ILayoutSetConstants.STACK_CONF_POSITION);
			confButton.setLayoutData(fdConfButton);
			FormData fdConfPos = stackBuilder
					.getPosition(ILayoutSetConstants.STACK_CONF_POS);
	...
	public void setActive(int newState) {
		...
		if (newState == AS_ACTIVE_FOCUS) {
			...
			confBg = stackBuilder
					.getImage(ILayoutSetConstants.STACK_CONF_BG_ACTIVE);
			...
			confImage = stackBuilder
					.getImage(ILayoutSetConstants.STACK_CONF_ACTIVE);
			tabOverflow = "tabOverflowActive"; //$NON-NLS-1$
			tabBgImage = stackBuilder
					.getImage(ILayoutSetConstants.STACK_TAB_BG_ACTIVE);
			changeStack(true);
		} else {
			if (!isStandalone()) {
		...
		if (adapter != null && adapter instanceof Map) {
			...
			if (active) {
				...
				bottomBorderImg = stackBuilder
						.getImage(ILayoutSetConstants.STACK_BORDER_BOTTOM);
				leftBorderImg = stackBuilder
						.getImage(ILayoutSetConstants.STACK_BORDER_LEFT);
				rightBorderImg = stackBuilder
						.getImage(ILayoutSetConstants.STACK_BORDER_RIGHT);
