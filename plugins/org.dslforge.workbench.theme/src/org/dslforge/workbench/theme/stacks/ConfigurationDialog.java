...
import org.dslforge.workbench.theme.managers.ItemData;
	...
	private void loadActionSettings(final Composite container) {
		...
		if (manager != null) {
			...
			if (manager instanceof ViewToolBarManager) {
				// manager.update( true );
				ItemData[] itemsData = ((ViewToolBarManager) manager)
						.getItemsData();
				for (int i = 0; i < itemsData.length; i++) {
					...
					if (itemData.getText() != null
							&& !itemData.getText().equals("")) { //$NON-NLS-1$
					...
					if (lastImageLabel != null) {
						fdImageLabel.top = new FormAttachment(lastImageLabel,
								OFFSET);
